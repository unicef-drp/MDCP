...
#html_fragment makes a slimmed-down version of the html. This solves the conflict of embedding an html into an html. The theme is applied in app:UI 
output:
 html_fragment:
    css: columns.css
    #theme: cerulean
...
wpp.dem.le.world <- wpp.dem |>
  filter(area=="WORLD") |>
  filter(year %in% 1950:2022) |> 
  select(year, le) |> 
  mutate(series = "World average") |> 
  rename(value=le)
  
wpp.dem.le.country <- wpp.dem |>
  filter(iso3 ==country) |>
  filter(year %in% 1950:2022) |> 
  select(year, le.m, le.f) |> 
  pivot_longer(cols=c(le.m, le.f), values_to = "value", names_to = "series") 

wpp.dem.le.country$series[wpp.dem.le.country$series=="le.m"] <- "Male"
wpp.dem.le.country$series[wpp.dem.le.country$series=="le.f"] <- "Female"

wpp.dem.le.country <- bind_rows(wpp.dem.le.country,wpp.dem.le.world)

wpp.dem.le.country$series <- factor(wpp.dem.le.country$series,
                                    levels = c("Female", "Male", "World average"))
##############
...
pop.df <- wpp.age.group.country |>
  ...
  left_join(wpp.dem.country |>
              select(year, 'Total fertility (live births per woman)' = tfr,
                     ...
                     'Under five mortality rate (deaths per 1 000 live births)' = u5mr,
                     'Female Life expectancy at birth (years)' = le.f,
                     'Male Life expectancy at birth (years)' = le.m), by = 'year') |>
...
```{r, out.width="50%", fig.show="hold", fig.ncol = 2, fig.cap=c('Figure 1', 'Figure 2')}
data2 <- mig.stock.ref |> 
  select(3,4,5,7) |> 
...
<font size = 3> **Trend in migrant stock (in thousands)<br>`r country.wpp.name`, 1990-2020** </em></font> 
...
::: {}
<font size = 3> **Age pyramid of migrants and non-migrants<br>`r country.wpp.name`, 2020** </em></font> 
...
emi.df <-  data1 |> filter(orig == country, dest %in% concerned_ctries) |>
  ...
  kable(align = c('l','r'), centering = FALSE)|> kable_classic(full_width = T) |>
  column_spec(1, bold = T, border_right = T,width = '8cm') 
...
The chart below shows the major migration corridors from and to `r country.wpp.name`. The light blue arrows show where emigrants from `r country.wpp.name` settled and the dark blue arrows show where immigrants in `r country.wpp.name` came from. The width of the arrow represents the number of migrants. Note, in `r country.wpp.name``r dt_mig_2020[5,2][[1]]` per cent of international migrants have refugee status.
...
dt_mmg_ctr <- Missing_Migrants_Global_Figures_allData_withCountries |> 
  ...
  group_by(`Incident year`, `Cause of death newclass`) |>
  summarise(pop = sum(`Number of Dead`, na.rm=TRUE), .groups='drop')
```
...
if(nrow(dt_mmg_ctr) != 0){
  fig.mmg <- ggplot(data =dt_mmg_ctr, aes(x = `Incident year`, y = pop, fill = `Cause of death newclass`)) + 
    geom_bar(stat = 'identity', position = 'stack', alpha = 0.8) +
    labs(fill = "Cause of Death",
      color = NULL,
      y = 'Deaths',x = NULL)+
    scale_fill_manual(values = causesColors)+
...
<font size = 1><em>***Source:*** IOM, 2023. Missing Migrants Protect, https://missingmigrants.iom.int/downloads. Accessed 29/08/2023.
...
***Technical note:*** Each incident of the MMP was assigned a country based on the location coordinates of the incident, including its location within country boundaries such as land and marine economic exclusive zones. The location coordinates pertain to where the death(s) occurred or where the body or bodies were discovered, although in some regions like the Mediterranean, the geographic coordinates are estimated due to imprecise information.
</em></font>


### Refugees 
**Refugees** are people who have fled war, violence, conflict or persecution and have (usually) crossed an international border to find safety in another country. Their status is defined in the [1951 Refugee Convention](https://www.unhcr.org/3b66c2aa10.html) and related protocols.

The number below refer to refugees and persons in refugee-like situations, they do not include asylum seekers.
...
```{r}
if(flag_UNHCR_edu){kable(dt_ref_edu_2019, align = c('l','r')) |> 
    kable_classic(full_width = T) |>
    column_spec(1, bold = T, border_right = T,width = '12cm') |>
...
if(country.wpp.name %in% dcr.ctry.ref$country){
  ...
  kable(dt10, align = c('l',rep('r',3))) |> kable_classic(full_width = T) |>
    column_spec(1, bold = T, border_right = T,width = '12cm')|>    column_spec(2:4, width = '3cm')
...
`r if(nrow(data.idp) > 0){"
<center><font size = 3>**Disaster events by hazard category and number of induced displacements (in thousands), 2008-2022 (in thousands)**</font></center>
"}`
...
if(nrow(data.idp) > 0){
  fig.adet <-  ggplot(data = data.adet, aes(x = start.date, y = idp.dis.new, size = idp.dis.new, color = hazard.cat)) +
    ...
    labs(color = NULL,
         y = 'Number of displacements (in 1 000)',x = NULL)+
...
```{r, message=F,warning=F,fig.align = "center",out.width="70%"}
...
Understanding the share of population by age groups in a country is essential for effective policymaking and resource allocation. This demographic information provides insights into the country's social structure, healthcare needs, and economic dynamics. A high proportion of children might indicate a need for investment in education and child welfare programs, while a growing elderly population could signal increased demand for healthcare services and pension systems. 
...
<font size = 3> **Share of population by age group<br>`r country.wpp.name`, `r yr_bg1`-`r yr_ed1`** </em></font> 
...
::: {}
<font size = 3> **Age Pyramid<br>`r country.wpp.name`, 2022 and 2050** </em></font> 
...
Knowing the fertility rates of a country is crucial for understanding its demographic dynamics and planning effective policies. Fertility rates provide insights into population growth, age distribution, and the potential future workforce. High fertility rates can strain resources and services, while low rates can lead to an aging population and potential labor shortages.
...
<font size = 3> **Total fertility rate<br>`r country.wpp.name`, 1950-2050** </em></font> 
...
::: {}
<font size = 3> **Number of births<br>`r country.wpp.name`, 1950-2050** </em></font> 
...
Understanding the life expectancy in a country is crucial for assessing the overall health and well-being of its population. It provides insights into the effectiveness of healthcare systems, quality of living conditions, and access to essential services. Disparities between genders can reveal underlying societal and cultural factors that may influence health outcomes. 
...
dt4[,2:6] <- round(dt4[,2:6], digits = 1)

colnames(dt4) <- c('Indicator', '2000' ,'2022' , '2050*' , '2022', '2022')
...
<font size = 3> **Female and Male life expectancy at birth<br>`r country.wpp.name`, 1950-2022** </em></font>  

</center> 
```{r, fig.align = "center",out.width="70%"}
if(!nrow(wpp.dem.le.country) == 0){
g <- ggplot(wpp.dem.le.country, aes(x=year, y = value, color=series)) +
  geom_line(size = 0.8) +
  #geom_point(size =2)+
  scale_color_manual(values = c(unicef_colors[1], unicef_colors[2], unicef_colors[9]))+
  blank_theme+
...
Child mortality rates are important indicators of a country's overall health and development. They provide a clear picture of the quality of healthcare, nutrition, sanitation, and living conditions for children. Monitoring child mortality rates helps identify regions or communities facing higher risks, enabling targeted interventions to reduce infant and child deaths. 
...
if(!nrow(dt.igme) == 0){
  ...
  igme$Indicator <- factor(igme$Indicator,
                           levels=c("Neonatal mortality rate", "Infant mortality rate", "Under-five mortality rate", "Mortality rate age 10-19"))
...
kable(dt7, align = c('l','r','r','r'))  |> kable_classic(full_width = T) |>
  column_spec(1, bold = T, border_right = T,width = '7cm') |>
...
Knowing the unemployment rates in a country, both for adults and youth, is crucial for assessing the overall health of its economy and the well-being of its citizens. High unemployment rates can indicate economic instability, inequality, and potential social tensions. Unemployment among youth is particularly significant as it affects their transition into the workforce, potentially leading to long-term negative effects on their career prospects and financial stability. 
...
ilo.df.blank$`2000`[1] <- ifelse(length(v1)==1,round(v1,1),NA)
...
ilo.df.blank$`2000`[2] <- ifelse(length(v1)==1,round(v1,1),NA)
...
ilo.df.blank$`2000`[3] <- ifelse(length(v1)==1,round(v1,1),NA)
...
ilo.df.blank$`2000`[4] <- ifelse(length(v1)==1,round(v1,1),NA)
...
<font size = 1><em>***Note:*** The unprecedented labour market shock created by the COVID-19 pandemic is difficult to assess by benchmarking against historical data (ILO). </em></font>
...
`r if(flag_wb){"<font size = 3>**GDP growth (annual %)**</font>"}`

```{r, out.width="90%"}
if(flag_wb){print(fig_chart7)}
```
:::

::: {}
`r if(flag_wb){"<font size = 3>**GNI per capita, Atlas method (current US$)**</font>"}`

```{r, out.width="90%"}
if(flag_wb){print(fig_chart8)}
```
:::
