...
input.unhcr <- file.path(path.basic, "Data/UNHCR")
input.idmc <- file.path(path.basic, "Data/IDMC/IDMC2023")
...
#load(file.path(input.unhcr,'GlobalTrends2022/unhcr2022_demographic_estimate.Rdata'))
...
uis <- read.csv(file.path(path.basic,'Data/UIS/GrossEnrolmentRatio.csv'),stringsAsFactors = F)



ref.school <- read_xlsx(file.path(input.unhcr,'GlobalTrends2020/Data Analysis 2019.xlsx'),range = 'Primary+Secondary!a3:ay15')
...
all_ctry <- all_ctry[!(all_ctry %in% c('TWN', "SHN", "FRO", "GGY", 
                                       "IMN", "JEY", "GIB", "VAT",
                                       "XKX", "BES", "VGB", "CYM",
                                       "MSR", "BLM", "MAF", "SXM",
                                       "TCA", "FLK", "BMU", "GRL",
                                       "SPM", "MHL", "NRU", "MNP",
                                       "PLW", "ASM", "COK", "NIU",
                                       "TKL", "TUV", "WLF"))] #which countries do not compute
...
render_profile <- function(country) {
  out <- tryCatch(
    {
      rmarkdown::render(
